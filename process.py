...
# oracleSql = f"""
# select emp_id, reward_ymd, appl_id, sta_hm, end_hm, reward_type from ehr2011060.tam2215 where del_yn='N'
# """

# OracleCursor.execute(oracleSql)

# #주말 초과근무 테이블
# weekend_table=pd.DataFrame()

# for line2 in OracleCursor:
...
for idx in range(len(origin_table)):
    ...
    if rows_origin['TYPE']=='1008': #초과근무
        ...
        if rows_origin['REWARD_TYPE']!='NULL': # reward type이 정의되어 있을 때(주말)
            ...
            merge_table.at[merge_index,'REWARD_ID']=rows_origin['APPL_ID']
            continue
...
print(len(merge_table.columns))

parameters='%s,'*31

for i in range(len(merge_table)):
    sql="INSERT INTO connect.ehr_cal values ("+str(i+1)+","+parameters[:-1]+")"
