...
import platform
import lib, merge, db, query, create
...
if os_name=='Windows':
    ...
    while True:
        interval_sta=input('commute log 생성 시작 기간을 설정해주세요. (YYYYMMDD)\n').strip()
        interval_end=input('commute log 생성 끝 기간을 설정해주세요. (YYYYMMDD)\n').strip()
...
try:
    ...
    mysql_cur.execute(f'delete from connect.ehr_cal where ymd>={start_day} and ymd<={end_day}')
    del start_day
    del end_day
