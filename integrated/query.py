...
LEFT JOIN (
    (SELECT EMP_ID,(SUM(CRT_D_CNT)-NVL(SUM(USED_D_CNT),0)) AS CNT,(SUM(CRT_D_CNT)-NVL(SUM(USED_D_CNT),0)) AS USED_CNT
...
LEFT JOIN (
    WITH Z as(
        ...
        inner join(
            ...
            AND SUBSTR(EMP_ID,5,1)!='5' 
            AND (SUBSTR(USE_END_YMD, 1, 4) = (SELECT TO_CHAR(SYSDATE, 'YYYY')FROM DUAL) OR SUBSTR(USE_END_YMD, 1, 4) = (SELECT TO_CHAR(SYSDATE, 'YYYY')FROM DUAL)+1)
            group by EMP_ID
...
pymysql_get_ehr_con_sql='''
    SELECT emp_id, `NAME`, SUM(
        CASE WHEN cal_overtime!='0000' then cast(floor(cast(cal_overtime AS INT)/100) as int)*60 + 
