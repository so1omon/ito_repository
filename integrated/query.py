...
SELECT NVL(B.EMP_ID,'NULL') AS emp_id,NVL(B.CD18, '0') AS over_std_time, NVL(D.CNT, '0') AS dayoff_std_time, 
NVL(D.CNT-E.VAL, '0') AS dayoff_rest_time,
NVL(F.EHC, '30') AS p_edu_std_time, NVL(G.E_SUM, '0') AS p_edu_admit_time, NVL(H.EHC, '20') AS i_edu_std_time, 
NVL(I.E_SUM, '0') AS i_edu_admit_time FROM EHR2011060.PA1010 A
left join (
    SELECT EMP_ID, T2.COND_CD18 AS CD18
    FROM EHR2011060.PA1020 T1,EHR2011060.SY5020 T2 
    WHERE T1.C_CD = '2011060' AND (SELECT TO_CHAR(SYSDATE, 'YYYYMMDD')FROM DUAL
...
LEFT JOIN (
    ...
    WHERE SUBSTR(USE_STA_YMD, 1, 4) = (SELECT TO_CHAR(SYSDATE, 'YYYY')FROM DUAL) AND HOLI_CLASS = '0001' GROUP BY EMP_ID
) D ON A.EMP_ID = D.EMP_ID
LEFT JOIN (
    SELECT EMP_ID, SUM(VAL) AS VAL 
    FROM EHR2011060.TAM5450 
    WHERE SUBSTR(YMD, 1, 4) = (SELECT TO_CHAR(SYSDATE, 'YYYY')FROM DUAL) AND ATTEND_CD = '0001' AND DEL_YN = 'N' GROUP BY EMP_ID
